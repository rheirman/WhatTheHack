...
namespace WhatTheHack.Harmony
{
    ...
    class RestUtility_CanUseBedEver
    {
        ...
        {
            if(!p.IsMechanoid() && (bedDef == WTH_DefOf.WTH_HackingTable || bedDef == WTH_DefOf.WTH_MechanoidPlatform || bedDef == WTH_DefOf.WTH_PortableChargingPlatform)){
                __result = false;
                return false;
            }
            return true;
    ...
    class RestUtility_CurrentBed
    {
        static bool Prefix(Pawn p, ref Building_Bed __result)
        {
            ...
            if (!(p.RaceProps != null && p.IsMechanoid()) || p.Map == null)
            {
                return true;
            }
