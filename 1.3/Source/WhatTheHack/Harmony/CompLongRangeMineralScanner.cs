...
namespace WhatTheHack.Harmony
{
        ...
        static bool Prefix(CompLongRangeMineralScanner __instance, Pawn worker, ref ThingDef ___targetMineable)
        {
            if (__instance!=null)
            {
                ...
                //    Traverse.Create(__instance).Field("daysWorkingSinceLastMinerals").SetValue(0f);
                    if (!TileFinder.TryFindNewSiteTile(out int tile, MinDistance, MaxDistance, true))
                        return false;
   ...
   public static class CompLongRangeMineralScanner_CompGetGizmosExtra
   {
        ...
        {
            return typeof(CompLongRangeMineralScanner).GetNestedTypes(AccessTools.all).FirstOrDefault((c) => c.Name == "<>c").GetMethods(AccessTools.all).FirstOrDefault(m => m.Name.Contains("CompGetGizmosExtra"));
        }
