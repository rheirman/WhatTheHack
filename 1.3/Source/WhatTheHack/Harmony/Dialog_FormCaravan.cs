...
namespace WhatTheHack.Harmony
{
    ...
    static class Dialog_FormCaravan_SelectApproximateBestTravelSupplies
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            var instructionsList = new List<CodeInstruction>(instructions);
            for (var i = 0; i < instructionsList.Count; i++)
            {
                ...
                {
                    yield return new CodeInstruction(OpCodes.Call, typeof(Dialog_FormCaravan_SelectApproximateBestTravelSupplies).GetMethod("AnimalOrWildManOrHacked"));
                }
                else
                {
                    yield return instruction;
